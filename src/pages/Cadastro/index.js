...
export default function Cadastro({ navigation }) {
  ...
  return (
    <Container>
      ...
      <Text>Crie uma senha com no mínimo 4 caracteres, ela será solicitada sempre que acessar seu APP.</Text>
      <TxtInput placeholder="Confirme senha" />
      <ViewSwitch>
        <SwitchCadastro onValueChange={() => setSwitchState(!switchState)} value={switchState} />
        <Text>Li e aceito os Termos de Uso</Text>
      </ViewSwitch>
      <BlueButton onPress={() => { navigation.navigate('Login'); }}><ButtonText>CRIE SUA CONTA</ButtonText></BlueButton>
    </Container>
  );
}
